...
class PseudoCharBoxBuilder_test:
    def __init__(
    ...
    def build_char_box(self, net, device, image, word_bbox, word, img_name=""):
        ...
        scores, net = self.inference_word_box(net,device,word_image)
        region_score = scores[0, :, :, 0].cpu().data.numpy()
        region_score = np.uint8(np.clip(region_score, 0, 1) * 255)
