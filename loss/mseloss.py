...
#             # positive_loss += torch.sum(pos_loss_region) / n_pos_pixel
...
#                     negative_loss += torch.sum(
#                         torch.topk(
#                             neg_loss_region.view(-1), int(neg_rto * n_pos_pixel)
#                         )[0]
#                     ) / (n_pos_pixel * neg_rto)
#             else:
#                 # only negative pixel
#                 negative_loss += (
#                     torch.sum(torch.topk(neg_loss_region.view(-1), 1000)[0]) / 1000
#                 )
#
...
#         char_loss = self.single_image_loss(loss_region, region_scores_label, neg_rto)
#         affi_loss = self.single_image_loss(
#             loss_affinity, affinity_scores_label, neg_rto
#         )
#
#         return char_loss + affi_loss
...
class Maploss_v3(nn.Module):
    ...
    def single_image_loss(self, pre_loss, loss_label, neg_rto, n_min_neg=1000):
        ...
        return total_loss
    def forward(self, region_scores_label, affinity_scores_label, region_scores_pre, affinity_scores_pre, mask, neg_rto, n_min_neg=1000):
        loss_fn = torch.nn.MSELoss(reduce=False, size_average=False)
