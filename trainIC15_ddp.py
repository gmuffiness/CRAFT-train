...
class Trainer(object):
    ...
    def train(self, buffer_dict):
        ...
        if self.config.train.real_dataset == 'icdar2015':
        ...
        while train_step < whole_training_step:
            ...
            for index, (
                ...
                if train_step % self.config.train.eval_interval == 0 and train_step != 0:
                    ...
                    # validation
                    self.iou_eval("icdar2015", train_step, save_param_path, buffer_dict["icdar2015"], craft)
                    self.cleval("icdar2015", train_step, save_param_path, craft)
...
def main():
    ...
    manager = mp.Manager()
    buffer1 = manager.list([None] * config["test"]["icdar2015"]["test_set_size"])
    buffer_dict = {"icdar2015":buffer1}
...
def main_worker(gpu, port, ngpus_per_node, config, buffer_dict, exp_name):
    ...
    if gpu == 0 and config["wandb_opt"]:
        wandb.init(project="craft-stage2", entity="gmuffiness", name=exp_name)
