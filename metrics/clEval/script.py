...
import metrics.clEval.rrc_evaluation_funcs as rrc_evaluation_funcs
import math
import os
...
def cleval_evaluation(gt_file, submit_file):
    ...
    root, extension = os.path.splitext(gt_file)
    if extension == '.zip':

        gt_files = load_zip_file(gt_file, 'label')
        submission_files = load_zip_file(submit_file, 'pred', True)

    else:
        gt_files = load_dir_file(gt_file,'label_cl')
        submission_files = load_dir_file(submit_file, 'pred', True)
    ...
    for file_idx in gt_files:
        if type(gt_files[file_idx]) is bytes:
            ...
            if file_idx in submission_files:
                det_file = submission_files[file_idx]
                if det_file is None:
                    det_file = ""
            else:
                det_file = ""

        #future = eval_single_result(gt_file, det_file)
...
def main(gt_dir, pred_dir, GT_BOX_TYPE='QUAD', PRED_BOX_TYPE='QUAD', TRANSCRIPTION=False, CONFIDENCES=False, CRLF=False):
    ...
    pr = result['method']['Detection']['precision']
    re = result['method']['Detection']['recall']
    ...
    result_dict["precision"] = pr
    result_dict["recall"] = re
    result_dict["hmean"] = pref
