...
class Trainer(object):
    ...
    def get_synth_loader(self):
        ...
        total_synth_dataset.append(synth_dataset)
        ...
        total_synth_dataset.append(ai_hub_dataset)
        # # # kor-syn
        data_path_kr = self.config.data_dir.synthtext_kor
        total_synth_dataset.extend(hierarchical_dataset(root=data_path_kr, config=self.config))



        dataloader = ConcatDataset(total_synth_dataset)

        synth_sampler = torch.utils.data.distributed.DistributedSampler(dataloader)
    ...
    def train(self):
        ...
        while train_step < whole_training_step:
            self.synth_sampler.set_epoch(epoch)
            for index, (
                image,
                region_image,
                affinity_image,
